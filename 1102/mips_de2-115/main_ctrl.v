...
//
      `BNE:    is_branch_ctrl_tmp = 3'b001;//等しくないとき分岐
//
//
//
...
//
  assign  jp = (((op_code == `J) && 1'b1) || ((op_code == `JAL) && 1'b1)) ? 1'b1 : 1'b0;
//
//
//
//
...
//
      `SW:     alu_op_tmp = 3'b000;//L504以降を参照
//
//
//
...
//
      `SW:     reg_write_enable_tmp = 1'b0;//L559参照
//
//
//
...
//
      `J:      reg_write_enable_tmp = 1'b0;//1'b1書き込み行う
//
//
//
...
//
      `ADDIU:  alu_ram_sel_s_tmp = 1'b0;//645参照
//
//
//
...
//
      `ADDIU:  reg_widx_sel1_s_tmp = 1'b0;//L688
//
//
//
...
//
      `JAL:    link_tmp = 1'b1;  // link == 1'b0： reg_widx_sel1_y を選択
                                 // link == 1'b1： レジスタファイル中の PC の値を記憶するレジスタ RA の idx, 31 を選択
//
//
//
